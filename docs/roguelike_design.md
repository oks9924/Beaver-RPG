...
- 자동 공격: 설정(기본 켬). 300px 안에 살아 있는 적이 있으면 걷는 중에도 마우스 방향으로 기본 공격을 공격 속도(준비+명중+후딜 시간)대로 계속한다. 기본 공격은 이동을 막지 않는다(`tempo.basic_attack_move_mult` 1.0, 2026-09-22). 공격 버튼을 누르고 있는 동안은 마우스 방향을 본다. 스킬·회복·건설과 대상이 있는 상호작용(F)은 기본 공격의 준비·후딜을 끊고 바로 나가고, 명중 순간에 누른 스킬은 `rules.input_buffer_sec`(0.4초) 동안 기억했다가 쓴다(2026-09-24). F 를 눌러도 대상이 없으면 자동 공격은 계속된다. 스킬·회복·건설·상호작용 입력이 있으면 그쪽이 먼저다(서버도 같은 우선순위). 탐색 모드·가방 열림·채팅 중에는 안 한다.
