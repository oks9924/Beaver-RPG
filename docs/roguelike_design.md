...
- 강화·재감정·분해 (2차, 2026-09-21): 재료는 **수액 결정** 하나. 분해(등급별 1/2/5/12/30 + 강화 단계)와 정예(+1)·보스(+3) 처치로 얻는다. 강화 +1~+5 는 기본 속성만 단계당 +10% (비용 3/5/8/12/18 × (1 + 등급 지수 × 0.5)). 재감정은 부가 속성 1줄을 골라 같은 등급 계층 안에서 다시 굴리며 다른 줄·직전 특성과 겹치지 않는다(고급 2 / 희귀 4 / 영웅 8 / 전설 15). 고유 특성은 재감정 불가, 장착 중인 장비는 분해 불가. 마을 메뉴 장비 탭의 상세 패널에서 처리하고 서버가 계정에 저장한다.
- 미구현: 제작 도안. 아이콘은 `docs/asset_request_v5.md`.
