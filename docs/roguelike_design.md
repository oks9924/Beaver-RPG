...
- 드랍 (2026-09-21 개정, Hero Siege 식 바닥 드랍 · **개인 드랍**): 적을 처치하면 **그 자리에** 장비가 떨어진다. 접속 중인 파티원마다 따로 굴리며, 떨어진 장비는 그 사람에게만 보이고 그 사람만 줍는다(경쟁 없음). 사람당 일반 6% · 정예 60%(고급 이상) · 보스 2개(희귀 이상). 버린 장비만 공용(모두에게 보이고 누구나 줍는다). 모든 장비(다른 직업 무기 포함)가 나올 수 있고 등급은 가중치 55/28/12/4/1 에 서약 열기 1당 +3%p, 깊이 1당 +1%p. 떨어질 때 시체에서 튀어나와 착지하고, 등급 색 바닥 빛 + 고급 이상 빛기둥(등급이 높을수록 굵고 길며 전설은 불꽃·고리)과 이름표가 붙는다. **밟으면 줍고**(창고 60개 상한, 차면 그 자리에 남고 알림) 파티 전원에게 알림이 간다. 방을 떠나면 바닥 장비는 사라진다. 방 클리어 보상 굴림은 없앴다(결정·도안은 유지).
