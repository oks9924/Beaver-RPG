...
## 효과음 (사내 합성, 2026-09-22): 장비 드랍·줍기
- `sfx.drop.{common,uncommon,rare,epic,legendary}`, `sfx.pickup` — `tools/gen_drop_sfx.py` 로 합성한 WAV(외부 샘플 없음). 등급이 높을수록 음이 많고 길다(둔탁한 착지 → 종 한 음 → 두 음 → 세 음 아르페지오 → 다섯 음 + 반짝임). 오디오 팩이 오면 같은 ID 로 교체하면 된다.
