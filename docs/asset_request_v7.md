# 에셋 요청 v7 — 방 지형 프리팹 조각용 소품·바닥 장식 (2026-09-22) — **납품·연결 완료 (GitHub Release assets-raw-v7, beaver_assets_v7.zip)**

연결 결과: 소품 18(바위 덩어리·통나무·긴 통나무 가로/세로·그루터기·덤불 × 3지역) + 바닥 장식 9 = 27 ID(변형 60프레임, select_frame)가 `assets/asset_manifest.json` 에 final 로 등록됐고(총 458 항목, 문제 0), `data/chunks.json` 의 역할 매핑이 자동으로 v7 ID 를 쓴다. 선택 항목 C(웅덩이 가장자리 9분할)는 미납품이라 웅덩이는 물 타일 사각으로 남는다. 팩 문서 사본은 `assets/packs/beaver_assets_v7/`.
