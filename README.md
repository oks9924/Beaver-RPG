...
원정(3지역, 지역마다 **던파식 방 격자** 5×3에 방 9~11개): 버들강 하류 → 검은 수액 습지 → 고대 뿌리댐. 왼쪽 열의 시작 방에서 오른쪽 끝의 보스 방까지 문으로 이어진 방을 걸어서 고른다. 전투방 클리어 → **보상 3지선다**(유물·스킬 강화·진화) → 같은 방의 **문이 열리고**, 문 앞에 파티가 모이면(전원 3초 · 과반 10초) 옆 방으로 이동. 옆길에는 정예·보물·상점·모닥불·사건 방이 있고, 클리어한 방은 되돌아갈 수 있다(적 재출현 없음). 미니맵(Tab)에 격자·현재 방·보스 위치가 보이고 이웃 방 유형은 옆에 가야 드러난다. 지역 보스 → 다음 지역 → … → 뿌리왕 → 결말(기본/정화). 방 목표는 섬멸·거점·장치·호위 + 정예전. 보상·메뉴·탐색(문 이동 대기) 상태마다 체크포인트가 저장되고, 안전 지점에서 **원정 중단**하면 모집판의 **이어하기**로 같은 자리에서 계속한다.

직업 5종(중복 가능): 수호목수(보호), 톱니전사(연타·열의), 솔방울사수(원거리·표식), 수액주술사(회복·속박·씨앗), 물길공학자(포탑·급류·댐·수압). 스킬마다 변형 2개와 런 중 진화, 직업 숙련 1~10 과 대체 특성 3개.

**영구 장비**(`data/equipment.json`, `data/affixes.json`): 방을 클리어하면 25%(정예·보스 확정)로 장비가 떨어져 마을 창고에 남는다. 무기는 직업당 2종(기본 공격이 바뀜: 통나무·대톱·투척기·방울총·렌치), 갑옷 3종, 장신구 4종. 등급 5단계(일반·고급·희귀·영웅·전설)로 부가 속성이 0/1/2/3/3줄 붙고 전설은 고유 특성 1개를 더 갖는다. 기본 속성은 지역 레벨로 오른다. 마을 메뉴 **장비** 탭에서 직업별 무기·갑옷·장신구 2개를 장착한다. 합산 상한이 있어 유물 빌드가 런의 중심으로 남는다.
