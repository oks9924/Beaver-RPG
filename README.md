...
현재 상태: **단계 0~4 구현 + 로그라이크 표준 규칙 + 던파식 던전 격자 + 영구 장비 + 속도감 조정 (v0.4.8)** — 전용 서버·영구 월드, 5직업, 3지역·보스 3종(독립 기믹 15개 × 1~4인 프로필), 일반 적 12종, 유물 36개, NPC 6명·퀘스트 15개, 마을 시설 5개, 난이도·접근성·튜토리얼·중단/이어하기, 서약(열기)·시간 위험도·디렉터 증원·정예 접두 5종·보상 희귀도/리롤·제단 사건 4종·휴식/숫돌·시간 문 보너스(`docs/roguelike_design.md`). **단계 5(실제 친구 4대 PC 검증)는 사용자가 수행**해야 하며 절차는 `docs/playtest_checklist.md`. 에셋팩 v1~v7(캐릭터·적·보스·기믹 장치·VFX·3지역 타일·소품·아이콘·초상·NPC·UI·오디오·장비 UI·걷기/공격 교체·지형 조각 소품)을 전부 연결했고 임시 항목은 없다(`docs/asset_plan.md`). 범위와 제한은 아래와 `docs/verification.md`.
...
조작: WASD 이동(바라보는 방향은 이동 키가 정한다), 마우스 조준(공격 중·스킬을 시작하는 순간 그 방향을 본다), 좌클릭 기본 공격(걷는 중에도 멈추지 않고 공격 속도대로 나간다. 가까운 적이 있으면 자동 공격, 설정에서 끔), Space 회피, I 가방(버리기·건네기), Q/E/R 스킬, F 상호작용(구조·갉기·장치·수문·기믹·NPC, 자동 공격 중에도 바로 된다), B 건설 / G 건설 종류, 1 회복, 중클릭 핑, Tab 큰 지도(마을에서는 메뉴 창: 원정 모집판·내실·장비·숙련·도감·퀘스트), Enter 채팅, Esc 설정(텍스트 크기·음량·흔들림·섬광 감소·타격 정지·키 재설정), F3 개발 화면, F11 전체화면.
